...
Constr = constraint(0,g_ee_x_str,0,'trajectory', xVars) + ... % x(0)
    ...
    constraint(0,'a3',0,'trajectory',xVars) + ...
    constraint(0,'a6',0,'trajectory',xVars) + ...
    constraint(0, 'a4', pi/2, 'trajectory', xVars) + ...
    constraint(0, 'a7', pi/2, 'trajectory', xVars) + ...
...
REV = [A1(end) A2(end) A3(end) A4(end) A5(end) A6(end) A7(end)];
csvwrite('../CSV Files/ReverseAngles.csv',REV);
